...
# TXTRecord, PTRRecord and ZoneForward are tier:core — only need API
# credentials (INFOBLOX_HOST, INFOBLOX_USER, INFOBLOX_PASS), no external
# infrastructure beyond the NIOS Grid Manager itself.
UPTEST_MANIFESTS_CORE = $(UPTEST_MANIFESTS_RECORD_AAAA),$(UPTEST_MANIFESTS_RECORD_CNAME),$(UPTEST_MANIFESTS_RECORD_MX),$(UPTEST_MANIFESTS_RECORD_PTR),$(UPTEST_MANIFESTS_RECORD_TXT),$(UPTEST_MANIFESTS_ZONE_DELEGATED),$(UPTEST_MANIFESTS_ZONE_FORWARD)
...
.PHONY: local-deploy e2e-preflight e2e.record-aaaa e2e.record-cname e2e.record-mx e2e.record-ptr e2e.record-txt e2e.zone-delegated e2e.zone-forward e2e-full
...
.PHONY: update-test.record-aaaa update-test.record-ptr update-test.record-txt update-test.zone-delegated update-test.zone-forward
