...
// Package v1alpha1 contains the Namespaced-scoped AAAARecord managed resource.
// API group: recordaaaa.infobloxnios.m.crossplane.io
package v1alpha1
...
// AAAARecordAwsRte53RecordInfo carries AWS Route 53 record information for a cloud-managed AAAARecord (mirrors the SDK's Awsrte53recordinfo struct).
type AAAARecordAwsRte53RecordInfo struct {
...
// AAAARecordCloudInfo carries Cloud API delegation/ownership information for a cloud-managed AAAARecord (mirrors the SDK's GridCloudapiInfo struct).
type AAAARecordCloudInfo struct {
	// The Cloud Platform Appliance to which authority of the object is delegated.
	DelegatedMember *AAAARecordCloudInfoDelegatedMember `json:"delegatedMember,omitempty"`
...
// AAAARecordCloudInfoDelegatedMember identifies the Grid member a cloud-managed AAAARecord's authority is delegated to (mirrors the SDK's Dhcpmember struct).
type AAAARecordCloudInfoDelegatedMember struct {
...
// AAAARecordMsAdUserData carries Microsoft Active Directory user information for an MS-managed AAAARecord (mirrors the SDK's MsserverAduserData struct).
type AAAARecordMsAdUserData struct {
	// The number of active users.
	ActiveUsersCount *int64 `json:"activeUsersCount,omitempty"`
}

// AAAARecordDiscoveredData carries DNS-discovery-feature data for an AAAARecord (mirrors the SDK's Discoverydata struct — 96 fields, all response-only; populated only when the NIOS Discovery feature is enabled and has scanned the host).
type AAAARecordDiscoveredData struct {
...
// AAAARecordParameters are the configurable fields of a AAAARecord.
type AAAARecordParameters struct {
	// Owner name in FQDN format for the AAAA record. Renaming changes the record's _ref.
	// +kubebuilder:validation:Required
	Name *string `json:"name"`
	// IPv6 address of the record. May be set statically or allocated dynamically from a CIDR at create time.
	// +kubebuilder:validation:Required
	...
	// DNS view in which the record resides, e.g. "external". Fixed at creation — WAPI ties the record's _ref to (view, zone, name). Confirmed absent from the UpdateAAAARecord SDK method signature.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:XValidation:rule="self == oldSelf",message="view is immutable after creation"
	View *string `json:"view"`
	// CIDR of the network from which to allocate the next available IP address (WAPI func:nextavailableip). Create-time-only — ignored on Update. Mutually exclusive with the static ipv6Addr field. When set, networkView is also required.
	...
	// Delete option: also remove the associated PTR record. Write-only — never present in the RecordAAAA struct returned by GetAAAARecord, so it is excluded from the AtProvider mirror.
	RemoveAssociatedPtr *bool `json:"removeAssociatedPtr,omitempty"`
}

// AAAARecordObservation holds the observed state of a AAAARecord.
// It mirrors every AAAARecordParameters field plus response-only fields
...
type AAAARecordObservation struct {
	...
	// Zone in which the record resides, e.g. "zone.com". Derived from name/view by WAPI — not a CreateAAAARecord parameter, so it has no ForProvider counterpart.
	...
	AwsRte53RecordInfo *AAAARecordAwsRte53RecordInfo `json:"awsRte53RecordInfo,omitempty"` // atProvider
	// Cloud API related information for this object (cloud-managed records only).
	CloudInfo *AAAARecordCloudInfo `json:"cloudInfo,omitempty"` // atProvider
	// Microsoft Active Directory user information (MS-managed records only).
	MsAdUserData *AAAARecordMsAdUserData `json:"msAdUserData,omitempty"` // atProvider
	// Discovered data for this AAAA record (DNS discovery feature).
	DiscoveredData *AAAARecordDiscoveredData `json:"discoveredData,omitempty"` // atProvider
}
...
type AAAARecordSpec struct {
	...
	// ForProvider holds the user-supplied parameters for this AAAARecord.
	// +kubebuilder:validation:Required
...
// AAAARecord is the Namespaced-scoped managed resource representing an Infoblox NIOS AAAARecord.
