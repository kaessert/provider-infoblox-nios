...
// API group: recordmx.infobloxnios.m.crossplane.io
package v1alpha1
...
// MXRecordAwsRte53RecordInfo carries AWS Route 53 record information for a cloud-managed MXRecord (mirrors the SDK's Awsrte53recordinfo struct).
type MXRecordAwsRte53RecordInfo struct {
...
// MXRecordCloudInfo carries Cloud API delegation/ownership information for a cloud-managed MXRecord (mirrors the SDK's GridCloudapiInfo struct).
type MXRecordCloudInfo struct {
	// The Cloud Platform Appliance to which authority of the object is delegated.
	DelegatedMember *MXRecordCloudInfoDelegatedMember `json:"delegatedMember,omitempty"`
...
// MXRecordCloudInfoDelegatedMember identifies the Grid member a cloud-managed MXRecord's authority is delegated to (mirrors the SDK's Dhcpmember struct).
type MXRecordCloudInfoDelegatedMember struct {
...
// MXRecordMsAdUserData carries Microsoft Active Directory user information for an MS-managed MXRecord (mirrors the SDK's MsserverAduserData struct).
type MXRecordMsAdUserData struct {
	// The number of active users.
	ActiveUsersCount *int64 `json:"activeUsersCount,omitempty"`
}

// MXRecordParameters are the configurable fields of a MXRecord.
type MXRecordParameters struct {
	// Owner name (FQDN) the MX record applies to.
	// +kubebuilder:validation:Required
	Name *string `json:"name"`
	// Mail exchanger hostname in FQDN format.
	// +kubebuilder:validation:Required
	MailExchanger *string `json:"mailExchanger"`
	// Preference value, 0-65535 — lower values are preferred.
	// +kubebuilder:validation:Required
...
// MXRecordObservation holds the observed state of a MXRecord.
// It mirrors every MXRecordParameters field plus response-only fields
...
type MXRecordObservation struct {
	...
	// Mail exchanger hostname in FQDN format.
	MailExchanger *string `json:"mailExchanger,omitempty"` // atProvider
	// Preference value, 0-65535 — lower values are preferred.
	...
	// Mail exchanger name in punycode format (derived from mailExchanger).
	DNSMailExchanger *string `json:"dnsMailExchanger,omitempty"` // atProvider
	...
	AwsRte53RecordInfo *MXRecordAwsRte53RecordInfo `json:"awsRte53RecordInfo,omitempty"` // atProvider
	// Cloud API related information for this object (cloud-managed records only).
	CloudInfo *MXRecordCloudInfo `json:"cloudInfo,omitempty"` // atProvider
	// Microsoft Active Directory user information (MS-managed records only).
	MsAdUserData *MXRecordMsAdUserData `json:"msAdUserData,omitempty"` // atProvider
}
...
type MXRecordSpec struct {
	...
	// ForProvider holds the user-supplied parameters for this MXRecord.
	// +kubebuilder:validation:Required
...
// MXRecord is the Namespaced-scoped managed resource representing an Infoblox NIOS MXRecord.
