...
// Package v1alpha1 contains the Namespaced-scoped PTRRecord managed resource.
// API group: recordptr.infobloxnios.m.crossplane.io
package v1alpha1
...
type PTRRecordAwsRte53RecordInfo struct {
...
// PTRRecordCloudInfo carries Cloud API delegation/ownership information for a cloud-managed PTRRecord (mirrors the SDK's GridCloudapiInfo struct).
type PTRRecordCloudInfo struct {
	// The Cloud Platform Appliance to which authority of the object is delegated.
	DelegatedMember *PTRRecordCloudInfoDelegatedMember `json:"delegatedMember,omitempty"`
...
// PTRRecordCloudInfoDelegatedMember identifies the Grid member a cloud-managed PTRRecord's authority is delegated to (mirrors the SDK's Dhcpmember struct).
type PTRRecordCloudInfoDelegatedMember struct {
...
// PTRRecordMsAdUserData carries Microsoft Active Directory user information for an MS-managed PTRRecord (mirrors the SDK's MsserverAduserData struct).
type PTRRecordMsAdUserData struct {
	// The number of active users.
	ActiveUsersCount *int64 `json:"activeUsersCount,omitempty"`
}

// PTRRecordDiscoveredData carries DNS-discovery-feature data for a PTRRecord (mirrors the SDK's Discoverydata struct — 96 fields, all response-only; populated only when the NIOS Discovery feature is enabled and has scanned the host).
type PTRRecordDiscoveredData struct {
...
// PTRRecordParameters are the configurable fields of a PTRRecord.
...
// It mirrors every PTRRecordParameters field plus response-only fields
...
type PTRRecordObservation struct {
	...
	// Domain name this PTR record points to, in FQDN format. Changing it updates the record's _ref (best-effort target — WAPI does not verify that the referenced A/AAAA record exists).
	Ptrdname *string `json:"ptrdname,omitempty"` // atProvider
	// PTR record name in FQDN (in-addr.arpa/ip6.arpa) format. Auto-derived from ipv4Addr/ipv6Addr when omitted; renaming changes the record's _ref.
	...
	// Target domain name in punycode format.
	DNSPtrdname *string `json:"dnsPtrdname,omitempty"` // atProvider
	...
	AwsRte53RecordInfo *PTRRecordAwsRte53RecordInfo `json:"awsRte53RecordInfo,omitempty"` // atProvider
	// Cloud API related information for this object (cloud-managed records only).
	CloudInfo *PTRRecordCloudInfo `json:"cloudInfo,omitempty"` // atProvider
	// Microsoft Active Directory user information (MS-managed records only).
	MsAdUserData *PTRRecordMsAdUserData `json:"msAdUserData,omitempty"` // atProvider
	// Discovered data for this PTR record (DNS discovery feature).
	DiscoveredData *PTRRecordDiscoveredData `json:"discoveredData,omitempty"` // atProvider
}
...
type PTRRecordSpec struct {
	...
	// ForProvider holds the user-supplied parameters for this PTRRecord.
	// +kubebuilder:validation:Required
...
// PTRRecord is the Namespaced-scoped managed resource representing an Infoblox NIOS PTRRecord.
...
// PTRRecordList contains a list of PTRRecord.
