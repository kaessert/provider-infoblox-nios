...
// Package v1alpha1 contains the Namespaced-scoped TXTRecord managed resource.
// API group: recordtxt.infobloxnios.m.crossplane.io
package v1alpha1
...
// TXTRecordAwsRte53RecordInfo carries AWS Route 53 record information for a cloud-managed TXTRecord (mirrors the SDK's Awsrte53recordinfo struct).
type TXTRecordAwsRte53RecordInfo struct {
...
// TXTRecordCloudInfo carries Cloud API delegation/ownership information for a cloud-managed TXTRecord (mirrors the SDK's GridCloudapiInfo struct).
type TXTRecordCloudInfo struct {
	// The Cloud Platform Appliance to which authority of the object is delegated.
	DelegatedMember *TXTRecordCloudInfoDelegatedMember `json:"delegatedMember,omitempty"`
...
// TXTRecordCloudInfoDelegatedMember identifies the Grid member a cloud-managed TXTRecord's authority is delegated to (mirrors the SDK's Dhcpmember struct).
type TXTRecordCloudInfoDelegatedMember struct {
...
// TXTRecordMsAdUserData carries Microsoft Active Directory user information for an MS-managed TXTRecord (mirrors the SDK's MsserverAduserData struct).
type TXTRecordMsAdUserData struct {
	// The number of active users.
	ActiveUsersCount *int64 `json:"activeUsersCount,omitempty"`
}

// TXTRecordParameters are the configurable fields of a TXTRecord.
type TXTRecordParameters struct {
	// Owner name (FQDN) the TXT record applies to. Renaming changes the record's _ref.
	// +kubebuilder:validation:Required
	Name *string `json:"name"`
	// Text content, up to 255 bytes per substring and 512 bytes total. Quote to preserve leading/trailing/embedded spaces. Changing this value changes the record's _ref.
	// +kubebuilder:validation:Required
...
// TXTRecordObservation holds the observed state of a TXTRecord.
// It mirrors every TXTRecordParameters field plus response-only fields
...
type TXTRecordObservation struct {
	...
	// Text content, up to 255 bytes per substring and 512 bytes total. Quote to preserve leading/trailing/embedded spaces. Changing this value changes the record's _ref.
	Text *string `json:"text,omitempty"` // atProvider
	// Comment for the record; maximum 256 characters.
	...
	// Zone in which the record resides, e.g. "zone.com". Derived from name/view by WAPI — not a CreateTXTRecord parameter, so it has no ForProvider counterpart.
	...
	AwsRte53RecordInfo *TXTRecordAwsRte53RecordInfo `json:"awsRte53RecordInfo,omitempty"` // atProvider
	// Cloud API related information for this object (cloud-managed records only).
	CloudInfo *TXTRecordCloudInfo `json:"cloudInfo,omitempty"` // atProvider
	// Microsoft Active Directory user information (MS-managed records only).
	MsAdUserData *TXTRecordMsAdUserData `json:"msAdUserData,omitempty"` // atProvider
}
...
type TXTRecordSpec struct {
	...
	// ForProvider holds the user-supplied parameters for this TXTRecord.
	// +kubebuilder:validation:Required
...
// TXTRecord is the Namespaced-scoped managed resource representing an Infoblox NIOS TXTRecord.
