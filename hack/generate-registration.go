...
// discoverProviderConfigs looks for apis/cluster/v1alpha1/register.go and
// apis/namespaced/v1alpha1/register.go, the indicators that the provider-wide
// ProviderConfig core packages exist (the auto-generated registration
// convention: these packages are always present and always scanned first,
// cluster before namespaced). Per the dual-scope ProviderConfig split, the
// legacy cluster-scoped ProviderConfig lives in the plain-group core package
// (apis/cluster/v1alpha1) and the namespaced ProviderConfig plus
// ClusterProviderConfig live in the .m.-group core package
// (apis/namespaced/v1alpha1). A missing package is simply omitted from the
// result.
func discoverProviderConfigs(modulePath string) []apiEntry {
	var entries []apiEntry
	for _, scope := range []string{"cluster", "namespaced"} {
		registerFile := filepath.Join("apis", scope, "v1alpha1", "register.go")
		if _, err := os.Stat(registerFile); os.IsNotExist(err) {
			continue
		}
		...
		entries = append(entries, apiEntry{
			...
			Resource:   "",
			Version:    version,
			Alias:      scope + providerName(modulePath) + version,
			ImportPath: modulePath + "/apis/" + scope + "/" + version,
		})
	}
	return entries
}
...
// writeAPIRegistration writes apis/zz_generated_register.go. pcEntries, when
// non-empty, are always emitted first (cluster before namespaced) and
// registered before any MR types so that ProviderConfig resolution never
// depends on MR registration order. The output is fully self-contained (the
// auto-generated registration convention): it declares its own
// runtime.SchemeBuilder aggregate and AddToScheme entrypoint rather than
// depending on any hand-written file.
func writeAPIRegistration(pcEntries []apiEntry, entries []apiEntry) error {
	...
	buf.WriteString("\n")
	for _, pc := range pcEntries {
		fmt.Fprintf(&buf, "\t%s %q\n", pc.Alias, pc.ImportPath)
	}
	...
	for _, pc := range pcEntries {
		fmt.Fprintf(&buf, "\t// Register %s-scope ProviderConfig types.\n", pc.Scope)
...
func main() {
	...
	pcEntries := discoverProviderConfigs(modulePath)
	apiEntries := discoverAPIPackages(modulePath)
	controllerEntries := discoverControllers(modulePath, apiEntries)

	if err := writeAPIRegistration(pcEntries, apiEntries); err != nil {
