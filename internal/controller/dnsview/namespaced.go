...
func (e *namespacedExternal) Observe(_ context.Context, cr *namespacedv1alpha1.DNSView) (managed.ExternalObservation, error) {
	...
	if err != nil {
		if isNotFound(err) {
			...
			found, searchErr := viewExistsByNaturalKey(e.conn, cr.Spec.ForProvider.Name)
			if searchErr != nil {
				return managed.ExternalObservation{}, errors.Wrap(searchErr, errObserveDNSView)
			}
			if found {
	...
	cr.Status.AtProvider = namespacedObservationFromFields(observed, externalID, strPtrOrNil(v.Ref), &v.IsDefault, cloudInfo)
...
// the external name.
func (e *namespacedExternal) Create(_ context.Context, cr *namespacedv1alpha1.DNSView) (managed.ExternalCreation, error) {
...
// custom view, a 404 on the stored _ref is not treated as already-deleted
// by itself — see deleteViewResolving404 — because the _ref is a derived
// handle that rotates whenever an identity field changes, and a stale
// handle 404s exactly like a genuinely deleted object.
func (e *namespacedExternal) Delete(_ context.Context, cr *namespacedv1alpha1.DNSView) (managed.ExternalDelete, error) {
...
func setupNamespacedDNSView(mgr ctrl.Manager, o controller.Options) error {
	...
	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*namespacedv1alpha1.DNSView](&namespacedConnector{
			kube:  mgr.GetClient(),
