...
func (m *mockWapiServer) seed(nv *ibclient.NetworkView) string {
	...
	if nv.Ref == "" {
		...
		nv.Ref = "networkview/test" + itoa(m.nextRef) + ":" + name
	}
	m.views[nv.Ref] = nv
	return nv.Ref
}
...
func TestObservePreCreateState(t *testing.T) {
	// externalName == cr.GetName() simulates NameAsExternalName's default
	// before Create() has run. observeRefFor maps this to "", so the
	// identity ladder resolves purely by identity search (uid-only),
	// which finds nothing against an empty backend, without ever
	// treating the CR's own Kubernetes name as a WAPI _ref.
	m := newMockWapiServer()
	srv := httptest.NewServer(m.handler())
	defer srv.Close()
	mc := newTestClient(t, srv)
...
func TestClusterObserveAdoptsUnstampedObjectAndForcesUpdate(t *testing.T) {
	...
	// No identity stamp at all — simulates an object that predates the
	// identity rollout.
	nv := &ibclient.NetworkView{Name: stringPtr("test-view"), Comment: stringPtr("c")}
	ref := m.seed(nv)
	...
	if obs.ResourceUpToDate {
		t.Fatalf("adopted object must never report up to date")
	}
}
...
func TestClusterCreateStampsIdentity(t *testing.T) {
	...
	if meta.GetExternalName(cr) == "my-view" {
	...
	defer m.mu.Unlock()
	for _, nv := range m.views {
		got, ok := nv.Ea[identity.EAKey]
		if !ok || got != testUIDCluster {
			t.Fatalf("expected identity stamp %q, got %v", testUIDCluster, nv.Ea)
		}
	}
}

func TestCreateNetworkViewRefusesEmptyUID(t *testing.T) {
	m := newMockWapiServer()
	srv := httptest.NewServer(m.handler())
	defer srv.Close()
	mc := newTestClient(t, srv)
...
// call — createNetworkView's guard trims before comparing, matching
...
func TestClusterCreateWhitespaceUIDFailsWithZeroMutatingRequests(t *testing.T) {
	...
	defer m.mu.Unlock()
	if len(m.views) != 0 {
		t.Errorf("Create: len(m.views) = %d, want 0 — a whitespace-only uid must not create anything", len(m.views))
	}
}
...
func TestNamespacedUpdatePrerequisiteAutoCreates(t *testing.T) {
	...
	cr := newNamespacedNetworkView("default", "my-view", ref, "ProviderConfig")
	cr.Spec.ForProvider.Comment = stringPtr("updated")
	kube := fake.NewClientBuilder().WithScheme(newTestScheme(t)).WithObjects(cr).Build()
...
// call — updateNetworkView's guard trims before comparing, matching
// identity.Resolve's ladder (see internal/clients/identity). Without
...
func TestClusterUpdateWhitespaceUIDFailsWithZeroMutatingRequests(t *testing.T) {
	...
	nv := &ibclient.NetworkView{Name: stringPtr("test-view"), Comment: stringPtr("old comment")}
	nv.Ea = identity.Stamp(nil, testUIDCluster)
	ref := m.seed(nv)
...
func TestClusterDeleteRefusesUnverifiedOwnership(t *testing.T) {
	...
	// No identity stamp — adopted, not proven.
	nv := &ibclient.NetworkView{Name: stringPtr("test-view")}
	ref := m.seed(nv)
...
func TestClusterConnectSuccess(t *testing.T) {
	...
	secret := credentialsSecret("ns", "creds", "user", "pass")
...
func TestNamespacedConnectUnsupportedKind(t *testing.T) {
	...
	cr := newNamespacedNetworkView("ns", "my-view", "", "SomethingElse")
...
func TestNamespacedConnectWithClusterProviderConfig(t *testing.T) {
	...
	c := &namespacedConnector{kube: kube, usage: resource.NewProviderConfigUsageTracker(kube, &namespacedpcv1alpha1.ProviderConfigUsage{})}
	cr := newNamespacedNetworkView("ns", "my-view", "", "ClusterProviderConfig")
...
func TestNewEmptyNetworkViewCorrectness(t *testing.T) {
	nv := newEmptyNetworkView()
	if nv.ObjectType() != "networkview" {
		t.Fatalf("expected ObjectType networkview, got %q", nv.ObjectType())
	}
	found := false
	for _, f := range nv.ReturnFields() {
		if f == "extattrs" {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected ReturnFields to include extattrs, got %v", nv.ReturnFields())
	}
}
...
func TestLateInitializeDoesNotLeakIdentityKeyIntoExtAttrs(t *testing.T) {
	nv := &ibclient.NetworkView{Name: stringPtr("test-view")}
	nv.Ea = identity.Stamp(ibclient.EA{"Site": "dc1"}, "some-uid")

	var comment *string
	var extAttrs map[string]string
	lateInitialize(&comment, &extAttrs, nv)
...
func TestIsUpToDateIgnoresIdentityEA(t *testing.T) {
	nv := &ibclient.NetworkView{Name: stringPtr("test-view")}
	nv.Ea = identity.Stamp(nil, "some-uid")

	if !isUpToDate(stringPtr("test-view"), nil, nil, nv) {
