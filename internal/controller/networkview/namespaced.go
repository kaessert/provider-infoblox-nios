...
// +kubebuilder:rbac:groups=networkview.infobloxnios.m.crossplane.io,resources=networkviews,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networkview.infobloxnios.m.crossplane.io,resources=networkviews/status,verbs=get;update;patch

// namespacedConnector implements managed.TypedExternalConnector[*namespacedv1alpha1.NetworkView].
...
func (c *namespacedConnector) Connect(ctx context.Context, cr *namespacedv1alpha1.NetworkView) (managed.TypedExternalClient[*namespacedv1alpha1.NetworkView], error) {
	...
	objMgr, conn, err := newClient(creds)
	if err != nil {
		return nil, err
	}
...
func (e *namespacedExternal) Observe(_ context.Context, cr *namespacedv1alpha1.NetworkView) (managed.ExternalObservation, error) {
	...
	cr.Status.AtProvider = namespacedv1alpha1.NetworkViewObservation{
		Name:      o.Name,
		Comment:   o.Comment,
		ExtAttrs:  o.ExtAttrs,
		Ref:       o.Ref,
...
func (e *namespacedExternal) Create(_ context.Context, cr *namespacedv1alpha1.NetworkView) (managed.ExternalCreation, error) {
	p := cr.Spec.ForProvider
...
// Delete removes the NetworkView. A 404 is treated as already-deleted
// (idempotent). Deleting the Grid's default NetworkView is rejected by
// the server as a terminal error — this controller does not special-case
// it; the error simply surfaces via the wrapped errDeleteNetworkView.
func (e *namespacedExternal) Delete(_ context.Context, cr *namespacedv1alpha1.NetworkView) (managed.ExternalDelete, error) {
	externalID := meta.GetExternalName(cr)
	if err := deleteNetworkView(e.objMgr, externalID); err != nil {
		if isNotFound(err) {
			return managed.ExternalDelete{}, nil
		}
		return managed.ExternalDelete{}, errors.Wrap(err, errDeleteNetworkView)
	}
	return managed.ExternalDelete{}, nil
}
...
var (
	_ managed.TypedExternalConnector[*namespacedv1alpha1.NetworkView] = &namespacedConnector{}
	_ managed.TypedExternalClient[*namespacedv1alpha1.NetworkView]    = &namespacedExternal{}
)

// setupNamespacedNetworkView wires the namespaced NetworkView reconciler
// with the controller-runtime manager. Called from SetupGated (gate
// callback) and Setup (immediate path) in controller.go.
func setupNamespacedNetworkView(mgr ctrl.Manager, o controller.Options) error {
	...
	if o.MetricOptions != nil {
		if err := mgr.Add(statemetrics.NewMRStateRecorder(
			...
			&namespacedv1alpha1.NetworkViewList{},
			o.MetricOptions.PollStateMetricInterval,
		)); err != nil {
			return errors.Wrap(err, "cannot register namespaced NetworkView state recorder")
		}
	}

	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*namespacedv1alpha1.NetworkView](&namespacedConnector{
			kube:  mgr.GetClient(),
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(namespacedv1alpha1.SchemeGroupVersion.WithKind("NetworkView")),
		opts...,
	)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&namespacedv1alpha1.NetworkView{}).
