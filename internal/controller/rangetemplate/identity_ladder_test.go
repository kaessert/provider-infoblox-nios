// Identity ladder matrix tests for the RangeTemplate MR controllers. See
// networkview's identity_ladder_test.go for the rationale shared across
// this wave. RangeTemplate's _ref embeds its name, and — as the package
// doc records — "no immutable fields are known for RangeTemplate", so a
// genuine Update()-triggered rename rotation is directly reachable (like
// the pilot ARecord resource, not a defensive no-op).
...
func TestNamespacedObserveResolvedUpToDate(t *testing.T) {
	...
	cr := newNamespacedRangeTemplate("ns", "my-template", ref, "ProviderConfig")
...
func TestNamespacedObserveNotFound(t *testing.T) {
	...
	cr := newNamespacedRangeTemplate("ns", "my-template", "rangetemplate/doesnotexist:my-template", "ProviderConfig")
	e := &namespacedExternal{objMgr: mc.Manager, conn: mc.Connector}
...
func TestNamespacedObserveRecoversRotatedRefAndPersistsAnnotation(t *testing.T) {
	...
	cr := newNamespacedRangeTemplate("ns", "my-template", "rangetemplate/stale:my-template", "ProviderConfig")
	e := &namespacedExternal{objMgr: mc.Manager, conn: mc.Connector}
...
func TestClusterObserveRefusesOnForeignIdentityTyped(t *testing.T) {
	...
	cr := newClusterRangeTemplate("my-template", ref)
...
func TestClusterObserveRefusesOnAmbiguousMatch(t *testing.T) {
	...
	m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-a"), Ea: identity.Stamp(nil, testUIDCluster)})
	m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-b"), Ea: identity.Stamp(nil, testUIDCluster)})

	cr := newClusterRangeTemplate("my-template", "rangetemplate/stale:my-template")
	e := &clusterExternal{objMgr: mc.Manager, conn: mc.Connector}
...
func TestNamespacedObserveRefusesOnAmbiguousMatch(t *testing.T) {
	...
	m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-a"), Ea: identity.Stamp(nil, testUIDNamespaced)})
	m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-b"), Ea: identity.Stamp(nil, testUIDNamespaced)})
...
func TestClusterObserveServerError(t *testing.T) {
	...
	cr := newClusterRangeTemplate("my-template", "rangetemplate/test1:my-template")
	e := &clusterExternal{objMgr: mc.Manager, conn: mc.Connector}
...
func TestClusterDeleteRefusesOnAmbiguousMatch(t *testing.T) {
	...
	refA := m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-a"), Ea: identity.Stamp(nil, testUIDCluster)})
	refB := m.seed(&ibclient.Rangetemplate{Name: stringPtr("template-b"), Ea: identity.Stamp(nil, testUIDCluster)})
...
func TestClusterDeleteRecoversRotatedRefAndDeletes(t *testing.T) {
	...
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.templates[realRef]; ok {
...
func TestClusterCreateSendsIdentityEAInSingleRequest(t *testing.T) {
	...
	cr := newClusterRangeTemplate("my-template", "my-template")
	...
	if len(m.templates) != 1 {
		t.Fatalf("Create: expected exactly one object created, got %d", len(m.templates))
	}
	for _, rec := range m.templates {
		if got := rec.Ea[identity.EAKey]; got != testUIDCluster {
...
func TestClusterCreateRefusesBlankUID(t *testing.T) {
	...
	if len(m.templates) != 0 {
		t.Fatalf("Create: expected zero mutating requests for a blank uid, got %d objects created", len(m.templates))
	}
}
...
func TestNamespacedCreateSendsIdentityEAInSingleRequest(t *testing.T) {
	...
	cr := newNamespacedRangeTemplate("ns", "my-template", "my-template", "ProviderConfig")
	...
	for _, rec := range m.templates {
		if got := rec.Ea[identity.EAKey]; got != testUIDNamespaced {
...
func TestClusterUpdateRefreshedExternalNamePersistsAcrossReGet(t *testing.T) {
	...
	cr := newClusterRangeTemplate("my-template", oldRef)
	...
	fetched := &clusterv1alpha1.RangeTemplate{}
...
func TestNamespacedUpdateRefreshedExternalNamePersistsAcrossReGet(t *testing.T) {
	...
	cr := newNamespacedRangeTemplate("ns", "my-template", oldRef, "ProviderConfig")
	kube := fake.NewClientBuilder().WithScheme(newTestScheme(t)).WithObjects(cr).Build()
	...
	fetched := &namespacedv1alpha1.RangeTemplate{}
...
func TestClusterObserveAtProviderExtAttrsIncludesIdentityKey(t *testing.T) {
	...
	rec := &ibclient.Rangetemplate{Name: stringPtr("test-template")}
	rec.Ea = identity.Stamp(ibclient.EA{"Site": "dc1"}, testUIDCluster)
	ref := m.seed(rec)
...
func TestClusterObserveSurfacesPrerequisiteErrorFromIdentitySearch(t *testing.T) {
	...
	cr := newClusterRangeTemplate("my-template", "")
