...
func (e *namespacedExternal) Observe(_ context.Context, cr *namespacedv1alpha1.AAAARecord) (managed.ExternalObservation, error) {
	...
	if err != nil {
		if isNotFound(err) {
			...
			found, searchErr := aaaaRecordExistsByNaturalKey(e.objMgr, cr.Spec.ForProvider.View, cr.Spec.ForProvider.Name, cr.Spec.ForProvider.IPv6Addr)
			if searchErr != nil {
				return managed.ExternalObservation{}, errors.Wrap(searchErr, errObserveAAAARecord)
			}
			if found {
...
func (e *namespacedExternal) Create(_ context.Context, cr *namespacedv1alpha1.AAAARecord) (managed.ExternalCreation, error) {
	p := cr.Spec.ForProvider
	rec, err := createAAAARecord(e.objMgr, p.Name, p.View, p.IPv6Addr, p.Comment, p.TTL, p.UseTTL, p.ExtAttrs, p.Cidr, p.NetworkView)
	if err != nil {
...
// (UNSTABLE _ref) — see clusterExternal.Update for the full rationale.
func (e *namespacedExternal) Update(ctx context.Context, cr *namespacedv1alpha1.AAAARecord) (managed.ExternalUpdate, error) {
	...
	// See clusterExternal.Update — UpdateAAAARecord always returns the
	// object's current _ref, and renaming or changing ipv6Addr changes
	// the _ref.
...
// Delete removes the AAAARecord. A 404 on the stored _ref is not treated
// as already-deleted by itself — see deleteAAAARecordResolving404 —
...
// deleted object.
func (e *namespacedExternal) Delete(_ context.Context, cr *namespacedv1alpha1.AAAARecord) (managed.ExternalDelete, error) {
	externalID := meta.GetExternalName(cr)
	p := cr.Spec.ForProvider
	if err := deleteAAAARecordResolving404(e.objMgr, externalID, p.View, p.Name, p.IPv6Addr); err != nil {
...
func setupNamespacedAAAARecord(mgr ctrl.Manager, o controller.Options) error {
	...
	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*namespacedv1alpha1.AAAARecord](&namespacedConnector{
			kube:  mgr.GetClient(),
