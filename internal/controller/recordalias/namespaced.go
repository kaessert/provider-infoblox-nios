...
// +kubebuilder:rbac:groups=recordalias.infobloxnios.m.crossplane.io,resources=aliasrecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=recordalias.infobloxnios.m.crossplane.io,resources=aliasrecords/status,verbs=get;update;patch

// namespacedConnector implements managed.TypedExternalConnector[*namespacedv1alpha1.AliasRecord].
...
func (c *namespacedConnector) Connect(ctx context.Context, cr *namespacedv1alpha1.AliasRecord) (managed.TypedExternalClient[*namespacedv1alpha1.AliasRecord], error) {
	...
	objMgr, conn, err := newObjectManager(creds)
	if err != nil {
		return nil, err
	}
...
func (e *namespacedExternal) Update(_ context.Context, cr *namespacedv1alpha1.AliasRecord) (managed.ExternalUpdate, error) {
	p := cr.Spec.ForProvider
	externalID := meta.GetExternalName(cr)
	...
	// See clusterExternal.Update — the update response always carries the
	// object's current _ref, and renaming (name is _ref-mutating) changes
	// it.
...
// Delete removes the AliasRecord. A 404 is treated as already-deleted
// (idempotent).
...
var (
	_ managed.TypedExternalConnector[*namespacedv1alpha1.AliasRecord] = &namespacedConnector{}
	_ managed.TypedExternalClient[*namespacedv1alpha1.AliasRecord]    = &namespacedExternal{}
)

// setupNamespacedAliasRecord wires the namespaced AliasRecord reconciler
// with the controller-runtime manager. Called from SetupGated (gate
// callback) and Setup (immediate path) in controller.go.
func setupNamespacedAliasRecord(mgr ctrl.Manager, o controller.Options) error {
	...
	if o.MetricOptions != nil {
		if err := mgr.Add(statemetrics.NewMRStateRecorder(
			...
			&namespacedv1alpha1.AliasRecordList{},
			o.MetricOptions.PollStateMetricInterval,
		)); err != nil {
			return errors.Wrap(err, "cannot register namespaced AliasRecord state recorder")
		}
	}

	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*namespacedv1alpha1.AliasRecord](&namespacedConnector{
			kube:  mgr.GetClient(),
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(namespacedv1alpha1.SchemeGroupVersion.WithKind("AliasRecord")),
		opts...,
	)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&namespacedv1alpha1.AliasRecord{}).
