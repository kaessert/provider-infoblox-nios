// Package recordcname implements the Crossplane controller for the
// Infoblox NIOS CNAMERecord managed resource. Like the ARecord
// controller, this wraps the official infoblox-go-client Go SDK
// directly — the SDK's ObjectManager exposes typed CRUD methods
// (CreateCNAMERecord/GetCNAMERecordByRef/UpdateCNAMERecord/
...
// than a whole CNAMERecordParameters value. The cluster and namespaced
// CNAMERecordParameters types are structurally identical (same field
...
func isUpToDate(name, canonical, comment *string, ttl *uint32, useTTL *bool, extAttrs map[string]string, rec *ibclient.RecordCNAME) bool {
	...
	if strOrEmpty(canonical) != strOrEmpty(rec.Canonical) {
		return false
	}
...
// ttl, useTtl, extAttrs) from the observed RecordCNAME into spec so
...
// observedCNAMERecord holds the primitive field values extracted from a
// WAPI RecordCNAME response. The cluster and namespaced
// CNAMERecordObservation types are structurally similar but are distinct
...
// call site.
type observedCNAMERecord struct {
	ID        string
	Name      *string
	Canonical *string
	Comment   *string
...
// observeFromRecordCNAME extracts the fields mirrored by
// CNAMERecordObservation (the full-mirror AtProvider convention) from a
// WAPI RecordCNAME response. The SDK's GetCNAMERecordByRef method
// requests only a fixed subset of fields by default (extattrs, canonical,
...
func observeFromRecordCNAME(externalID string, rec *ibclient.RecordCNAME) observedCNAMERecord {
	o := observedCNAMERecord{
		ID:        externalID,
		Name:      rec.Name,
...
// deleteCNAMERecord issues the WAPI delete call.
...
// SetupGated registers both the cluster-scoped and namespaced CNAMERecord
...
func SetupGated(mgr ctrl.Manager, o controller.Options) error {
	o.Gate.Register(func() {
		if err := setupClusterCNAMERecord(mgr, o); err != nil {
			panic(errors.Wrap(err, "cannot setup cluster CNAMERecord controller"))
		}
	}, clusterv1alpha1.SchemeGroupVersion.WithKind("CNAMERecord"))

	o.Gate.Register(func() {
		if err := setupNamespacedCNAMERecord(mgr, o); err != nil {
			panic(errors.Wrap(err, "cannot setup namespaced CNAMERecord controller"))
		}
	}, namespacedv1alpha1.SchemeGroupVersion.WithKind("CNAMERecord"))

	return nil
}

// Setup starts both the cluster-scoped and namespaced CNAMERecord
...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	...
	return setupNamespacedCNAMERecord(mgr, o)
}
