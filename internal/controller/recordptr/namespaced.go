...
func (e *namespacedExternal) Observe(_ context.Context, cr *namespacedv1alpha1.PTRRecord) (managed.ExternalObservation, error) {
	...
	if err != nil {
		if isNotFound(err) {
			...
			found, searchErr := ptrRecordExistsByNaturalKey(e.objMgr, cr.Spec.ForProvider.View, cr.Spec.ForProvider.Ptrdname, cr.Spec.ForProvider.Name, cr.Spec.ForProvider.IPv4Addr, cr.Spec.ForProvider.IPv6Addr)
			if searchErr != nil {
				return managed.ExternalObservation{}, errors.Wrap(searchErr, errObservePTRRecord)
			}
			if found {
...
// deleted object.
func (e *namespacedExternal) Delete(_ context.Context, cr *namespacedv1alpha1.PTRRecord) (managed.ExternalDelete, error) {
	externalID := meta.GetExternalName(cr)
	p := cr.Spec.ForProvider
