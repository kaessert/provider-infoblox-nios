...
func (e *clusterExternal) Update(_ context.Context, cr *clusterv1alpha1.ZoneDelegated) (managed.ExternalUpdate, error) {
	p := cr.Spec.ForProvider
	externalID := meta.GetExternalName(cr)
...
// Delete removes the ZoneDelegated. A 404 is treated as already-deleted
// (idempotent).
func (e *clusterExternal) Delete(_ context.Context, cr *clusterv1alpha1.ZoneDelegated) (managed.ExternalDelete, error) {
	externalID := meta.GetExternalName(cr)
	if err := deleteZoneDelegated(e.objMgr, externalID); err != nil {
		if isNotFound(err) {
			return managed.ExternalDelete{}, nil
		}
		return managed.ExternalDelete{}, errors.Wrap(err, errDeleteZoneDelegated)
	}
	return managed.ExternalDelete{}, nil
}
...
func setupClusterZoneDelegated(mgr ctrl.Manager, o controller.Options) error {
	...
	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*clusterv1alpha1.ZoneDelegated](&clusterConnector{
			kube:  mgr.GetClient(),
