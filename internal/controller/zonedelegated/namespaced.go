...
// the cluster-scoped Observe for why hasZoneSerial is always false here.
func (e *namespacedExternal) Observe(ctx context.Context, cr *namespacedv1alpha1.ZoneDelegated) (managed.ExternalObservation, error) {
	...
	upToDate := isUpToDate(namespacedDelegateToSDK(p.DelegateTo), p.Comment, p.NsGroup, p.Disable, p.Locked, p.UseDelegatedTTL, p.DelegatedTTL, p.ExtAttrs, rec) && !res.adopted
...
func (e *namespacedExternal) Create(ctx context.Context, cr *namespacedv1alpha1.ZoneDelegated) (managed.ExternalCreation, error) {
	p := cr.Spec.ForProvider
	...
	rec, err := createZoneDelegated(e.objMgr, p.Fqdn, p.View, p.ZoneFormat, p.Comment, p.NsGroup, p.Disable, p.Locked, p.UseDelegatedTTL, p.DelegatedTTL, namespacedDelegateToSDK(p.DelegateTo), p.ExtAttrs, uid)
	if err != nil {
...
func (e *namespacedExternal) Update(ctx context.Context, cr *namespacedv1alpha1.ZoneDelegated) (managed.ExternalUpdate, error) {
	...
	rec, err := updateZoneDelegated(e.objMgr, externalID, p.Comment, p.NsGroup, p.Disable, p.Locked, p.UseDelegatedTTL, p.DelegatedTTL, namespacedDelegateToSDK(p.DelegateTo), p.ExtAttrs, string(cr.GetUID()))
	if err != nil {
...
func (e *namespacedExternal) Delete(ctx context.Context, cr *namespacedv1alpha1.ZoneDelegated) (managed.ExternalDelete, error) {
	externalID := meta.GetExternalName(cr)
...
func setupNamespacedZoneDelegated(mgr ctrl.Manager, o controller.Options) error {
	...
	opts := []managed.ReconcilerOption{
		managed.WithTypedExternalConnector[*namespacedv1alpha1.ZoneDelegated](driftdetection.WrapConnector[*namespacedv1alpha1.ZoneDelegated](&namespacedConnector{
			kube:     mgr.GetClient(),
