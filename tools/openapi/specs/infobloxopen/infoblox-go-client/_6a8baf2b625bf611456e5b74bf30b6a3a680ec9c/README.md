...
Files pinned (see `checksums.txt` for SHA-256 sums). Each is stored with a
`.txt` suffix appended to its original upstream filename — the content is
the unmodified upstream Go source (byte-for-byte, verified by the checksum),
but the `.txt` suffix keeps it out of this repository's own `.go` build
graph and out of Go-tooling filesystem walks (`gofmt`/`goimports` operate on
literal `.go` files, and the pinned copy intentionally does not follow this
repository's formatting conventions since it is not this repository's code):

- `object_manager.go.txt` — the `IBObjectManager` interface: the typed
...
- `objects.go.txt` — hand-written struct definitions for a handful of object
  types (`Network`, `NetworkContainer`, `FixedAddress`, `EA`, etc.) with
  runtime-selected object types (IPv4 vs IPv6 variants).
- `objects_generated.go.txt` — the bulk of the WAPI object model, generated
  from the NIOS WAPI schema documentation. Struct field doc comments are the
...
mkdir -p <repo>/tools/openapi/specs/infobloxopen/infoblox-go-client/<new-commit>
for f in objects.go objects_generated.go object_manager.go; do
  cp "$f" "<repo>/tools/openapi/specs/infobloxopen/infoblox-go-client/<new-commit>/$f.txt"
done
cd <repo>/tools/openapi/specs/infobloxopen/infoblox-go-client/<new-commit>
sha256sum objects.go.txt objects_generated.go.txt object_manager.go.txt > checksums.txt
```
