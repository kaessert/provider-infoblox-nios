...
// aaaaRecord returns the AAAARecord resource descriptor.
//
// Source: tools/openapi/inventory.md, "### AAAARecord" section (fields
...
// (inventory.md records this as "Live-verified: no" and infers delete
// behavior from ARecord, the identical WAPI record family verified
// 2026-07-28).
//
// AAAARecord is the IPv6 counterpart of ARecord — same shape, same
// lifecycle, same SDK wrapper pattern, differing only in the address field
// (ipv6Addr instead of ipv4Addr) and the SDK method/struct names
// (CreateAAAARecord/GetAAAARecord/UpdateAAAARecord/DeleteAAAARecord,
// RecordAAAA).
//
// External-name strategy: server-assigned (the WAPI `_ref` returned by
// CreateAAAARecord).
//
// Immutable fields: `view` is a CreateAAAARecord parameter (netView/dnsView)
// absent from UpdateAAAARecord's parameter list — confirmed against the SDK
// interface signatures (same shape as CreateARecord/UpdateARecord).
//
// `zone` is documented in inventory.md as immutable too, but it is NOT a
// CreateAAAARecord parameter at all — WAPI derives it from name+view. It
// therefore has no ForProvider representation and no CEL rule is emitted
// for it; it is AtProvider-only (see FieldDef.Immutable doc).
//
// `disable` appears on the underlying WAPI object but is NOT a parameter of
// CreateAAAARecord/UpdateAAAARecord (see inventory.md's Full Schema Notes
// for AAAARecord) — the SDK's ObjectManager wrapper cannot set it, so it is
// omitted from this catalog entirely rather than cataloged as a field with
// no working setter.
//
// No cross-resource references: AAAARecord is not a reference target or
// source for any other resource cataloged so far.
func aaaaRecord() ResourceDescriptor {
	return ResourceDescriptor{
		Kind:                 "AAAARecord",
		Slug:                 "recordaaaa",
		ClusterGroup:         clusterGroup("recordaaaa"),
		NamespacedGroup:      namespacedGroup("recordaaaa"),
		...
		Fields: []FieldDef{
			{
				...
				Required:    true,
				Description: "Owner name in FQDN format for the AAAA record. Renaming changes the record's _ref.",
			},
			{
				Name:        "IPv6Addr",
				JSONName:    "ipv6Addr",
				...
				Scope:       FieldScopeBoth,
				Required:    true,
				Description: "IPv6 address of the record. May be set statically or allocated dynamically from a CIDR at create time.",
			},
			{
				Name:        "Comment",
				JSONName:    "comment",
			...
			},
			{
				...
				Description:         "Delete option: also remove the associated PTR record. Write-only — never present in the RecordAAAA struct returned by GetAAAARecord, so it is excluded from the AtProvider mirror.",
			},
			{
				Name:        "Ref",
				JSONName:    "ref",
			...
			},
			{
				...
				Scope:       FieldScopeResponse,
				Immutable:   true,
				Description: "Zone in which the record resides, e.g. \"zone.com\". Derived from name/view by WAPI — not a CreateAAAARecord parameter, so it has no ForProvider counterpart.",
			},
			{
				Name:        "DNSName",
				JSONName:    "dnsName",
			...
			},
			{
				...
				GoType:      "*AAAARecordAwsRte53RecordInfo",
				Scope:       FieldScopeResponse,
			...
			},
			{
				...
				GoType:      "*AAAARecordDiscoveredData",
				Scope:       FieldScopeResponse,
				Description: "Discovered data for this AAAA record (DNS discovery feature).",
			},
		},
		NestedTypes: []NestedTypeDef{
			{
				TypeName:    "AAAARecordAwsRte53RecordInfo",
				Description: "carries AWS Route 53 record information for a cloud-managed AAAARecord (mirrors the SDK's Awsrte53recordinfo struct).",
				Fields: []FieldDef{
			...
			{
				TypeName:    "AAAARecordCloudInfo",
				Description: "carries Cloud API delegation/ownership information for a cloud-managed AAAARecord (mirrors the SDK's GridCloudapiInfo struct).",
				Fields: []FieldDef{
					{Name: "DelegatedMember", JSONName: "delegatedMember", GoType: "*AAAARecordCloudInfoDelegatedMember", Description: "The Cloud Platform Appliance to which authority of the object is delegated."},
			...
			{
				TypeName:    "AAAARecordCloudInfoDelegatedMember",
				Description: "identifies the Grid member a cloud-managed AAAARecord's authority is delegated to (mirrors the SDK's Dhcpmember struct).",
				Fields: []FieldDef{
			...
			{
				TypeName:    "AAAARecordMsAdUserData",
				Description: "carries Microsoft Active Directory user information for an MS-managed AAAARecord (mirrors the SDK's MsserverAduserData struct).",
				Fields: []FieldDef{
					{Name: "ActiveUsersCount", JSONName: "activeUsersCount", GoType: goTypeInt64, Description: "The number of active users."},
				},
			},
			{
				TypeName:    "AAAARecordDiscoveredData",
				Description: "carries DNS-discovery-feature data for an AAAARecord (mirrors the SDK's Discoverydata struct — 96 fields, all response-only; populated only when the NIOS Discovery feature is enabled and has scanned the host).",
				Fields: []FieldDef{
